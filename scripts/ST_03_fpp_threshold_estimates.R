...
calc_threshold <- function(t) {
  hit_10 <- buffers$perc_forest_2010 >= t
  hit_22 <- buffers$perc_forest_2022 >= t

  pop_sample_10  <- sum(buffers$fpp_2010[hit_10], na.rm = TRUE)
  area_sample_10 <- sum(buffers$area_km2[hit_10], na.rm = TRUE)
  dens_hat_10    <- ifelse(area_sample_10 > 0, pop_sample_10 / area_sample_10, NA_real_)
  pop_est_10     <- dens_hat_10 * mean(hit_10, na.rm = TRUE) * area_bioma_km2

  pop_sample_22  <- sum(buffers$fpp_2022[hit_22], na.rm = TRUE)
  area_sample_22 <- sum(buffers$area_km2[hit_22], na.rm = TRUE)
  dens_hat_22    <- ifelse(area_sample_22 > 0, pop_sample_22 / area_sample_22, NA_real_)
  pop_est_22     <- dens_hat_22 * mean(hit_22, na.rm = TRUE) * area_bioma_km2

  n_buffers  <- sum(hit_22, na.rm = TRUE)
  mean_perc  <- ifelse(n_buffers > 0, mean(buffers$perc_forest_2022[hit_22], na.rm = TRUE), NA_real_)
  sd_perc    <- ifelse(n_buffers > 1, sd(buffers$perc_forest_2022[hit_22],   na.rm = TRUE), NA_real_)
...
doc <- read_docx() %>%
  body_add_par(
    paste0("Supplementary Table 3. Estimated number of forest-proximate rural population (FPP) by ",
           "extrapolating to the entire Caatinga biome, the FPP in landscapes with different minimum ",
           "thresholds of forest cover."),
    style = "Normal"
  ) %>%
