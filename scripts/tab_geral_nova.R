...
nasc_2017 %>% 
  ...
  filter(code_mun != "") %>%
  select(code_mun, taxa_u5mort_2017) %>%
...
nasc_2022 %>% 
  ...
  filter(code_mun != "") %>%
  select(code_mun, taxa_u5mort_2022) %>%
  glimpse -> taxa_u5mort_2022

##cisterns----
cisternas_consumo_mds_2010_2022 %>% 
  select(code_mun, year, cisternas_acumulado) %>% 
...
cisternas_consumo_outros_2010_2022 %>% 
  select(code_mun, year, cistern_acumulado) %>% 
  filter(year == 2010 | year == 2017 | year == 2022) %>% 
  pivot_wider(id_cols = code_mun,
              names_from = year,
...
cisternas_producao_mds_2010_2022 %>% 
  select(code_mun, year, cisternas_producao_acumulado) %>% 
  filter(year == 2010 | year == 2017 | year == 2022) %>% 
  pivot_wider(id_cols = code_mun,
              names_from = year,
...
cisternas_consumo_mds %>% 
  ...
  mutate(across(everything(), ~replace_na(., 0))) %>%
  mutate(cisternas_2010 = cisternas_acumulado_2010.x + cisternas_acumulado_2010.y + cisternas_acumulado_2010,
...
##Irrigation----
irrigation %>% 
  select(mun_cd_atual, aa_ref, retirada_TOTAL_m3_s) %>% 
...
iliminacao_2010_2022 %>% 
  select(-name_mun) %>% 
  mutate(code_mun = as.factor(code_mun),
         public_light_2010 = as.double(public_light_2010)) %>% 
  glimpse -> iluminacao
